    ...
    private static ManagerSettings GetManagerSettings(IServiceProvider p)
    {
        ...
        try
        {
            ...
            if (File.Exists(settingsPath))
            {
                ...
                logger.LogDebug("Read settings file from '{SettingsPath}' with {EnabledProfiles} enabled profiles.", settingsPath, settings.EnabledProfiles);
                return settings;
            }
            else if (Directory.Exists(directoryName))
            {
                ...
                try
                {
                    ...
                    logger.LogWarning("Settings file at '{SettingsPath}' did not exist and was created", settingsPath);
                }
                catch (Exception ex)
                {
                    logger.LogError("Settings file at '{SettingsPath}' did not exist and could not be created: {ErrorMessage}", settingsPath, ex.Message);
                }
                return settings;
            }
            else
        ...
        {
            logger.LogError(new EventId(2002, "SettingsException"), "Error reading settings from '{SettingsPath}': {ErrorMessage}", settingsPath, ex.Message);
            throw;
        }
    }
